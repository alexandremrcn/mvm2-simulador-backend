...
public class TirJurosComposto extends TaxaInternaDeRetorno {
	...
	public double calcTaxaInternaDeRetorno()
	{
		...
		if (resultValue == true)
		{
			...
			double taxaInicialDia = 1 + (taxaInicialMes / 100);
			taxaInicialDia = Math.pow(taxaInicialDia, (double) (1.0 / 30.0));
			taxaInicialDia = taxaInicialDia - 1;
			taxaInicialDia = taxaInicialDia * 100;
			
			double taxaInternaDia = taxaInicialDia;
			
			ValorPresente vlPresente = new ValorPresenteJrComposto();
			...
			if (vlFator006 != 0.000000000)
			{
				...
				vlFator010 = vlFator002 / vlFator009;
			...
			taxaInternaDeRetorno =  1 + (taxaInternaDia / 100);
			taxaInternaDeRetorno = Math.pow(taxaInternaDeRetorno, (double) super.getBaseTaxa());
			taxaInternaDeRetorno = taxaInternaDeRetorno - 1;
			taxaInternaDeRetorno = taxaInternaDeRetorno * 100;
